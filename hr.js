...
const create_ticket = async(ticket) => {
        const url="https://newaccount1641286036293.freshdesk.com/api/v2/tickets";
        const response= await fetch(url, {
            method:'POST',
            headers: {
                "Authorization": "Basic " + btoa("Tp2JGk6fwxOsfIi93G3F" + ":x"),
...
sboxbtn.addEventListener("click", async()=>{
    ...
    const url=`https://newaccount1641286036293.freshdesk.com/api/v2/tickets/${ticket_id}?include=requester`;
...
const update_ticket = async(ticket) => {
    const url=`https://newaccount1641286036293.freshdesk.com/api/v2/tickets/${ticket_no}`;
    const response= await fetch(url, {
        method:'PUT',
        headers: {
            "Authorization": "Basic " + btoa("Tp2JGk6fwxOsfIi93G3F" + ":x"),
...
const del_ticket = async(T_number) => {
    const url=`https://newaccount1641286036293.freshdesk.com/api/v2/tickets/${T_number}`;
    const response= await fetch(url, {
        method:'DELETE',
        headers: {
            "Authorization": "Basic " + btoa("Tp2JGk6fwxOsfIi93G3F" + ":x"),
